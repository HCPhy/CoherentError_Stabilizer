...
for d = 1: l
    ...
    domainAx = mod(domainA0 + disp - 1, bit_num) + 1;
    domainBx = mod(domainAx + bit_num/2 - 1, bit_num) + 1;

    domainAz = domainAx + bit_num;
    domainBz = domainBx + bit_num;

    domainA = [domainAx, domainAz];
    domainB = [domainBx, domainBz];

    domainAB = [domainA, domainB];


    matA = xz_mat(:, domainA);
    matB = xz_mat(:, domainB);
    matAB = xz_mat(:, domainAB);
